...
- 시스템은 해당 브랜드의 상세 정보를 반환한다. (대표 상품 5개 제공)
...
- 존재하지 않는 상품 ID로 요청 시: 404 Not Found
- X-USER-ID 헤더 값이 존재하지 않는 회원 아이디인 경우: 401 Unauthorized
- X-USER-ID 헤더가 없는 경우 : 400 Bad Request
...
- X-USER-ID 헤더가 없는 경우 400 응답을 반환하는가?
